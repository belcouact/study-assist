#!/usr/bin/env python3
"""
Local API server for SVG data
This provides a local API endpoint that mimics the database structure
"""
...
class SVGAPIHandler(BaseHTTPRequestHandler):
    def do_OPTIONS(self):
    ...
    def do_GET(self):
        ...
        if len(path_parts) >= 4 and path_parts[0] == 'functions' and path_parts[1] == 'api':
            self.handle_api_request(path_parts, parsed_path.query)
        else:
            # Serve static files
            self.serve_static_file(parsed_path.path)

    def handle_api_request(self, path_parts, query_string):
        """Handle API requests"""
        try:
            # Extract action and table from path
            # Expected format: /functions/api/db/query/fa-svg
            if len(path_parts) >= 4:
                action = path_parts[2]  # 'db' or other actions
                sub_action = path_parts[3] if len(path_parts) > 3 else None
                table = path_parts[4] if len(path_parts) > 4 else None
                
                # Parse query parameters
                query_params = urllib.parse.parse_qs(query_string)
                database = query_params.get('database', ['default'])[0]
                
                if action == 'db' and sub_action == 'query' and table == 'fa-svg':
                    self.handle_svg_query()
                else:
                    self.send_error_response(400, "Invalid API endpoint")
            else:
    ...
    def handle_svg_query(self):
        """Handle SVG data query"""
        # Sample SVG data matching the database structure
        svg_data = [
            {
                "Name": "home",
                "Category": "solid",
...
def main():
    port = 8000
    server_address = ('', port)
    httpd = HTTPServer(server_address, SVGAPIHandler)
    
    print(f"Starting local API server on port {port}")
    print(f"Open http://localhost:{port}/svg-viewer.html to view the SVG viewer")
    print(f"API endpoint: http://localhost:{port}/functions/api/db/query/fa-svg")
