...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // Process each role in the dialog
    for (const role of requestData.dialog) {
      ...
      for (const line of role.lines) {
        ...
        console.log(`Processing line for ${roleName}: "${text.substring(0, 30)}${text.length > 30 ? '...' : ''}"`);
        ...
        try {
          ...
          audioClips.push(audioBytes);
          console.log(`Added audio clip for ${roleName}, length: ${audioBytes.length} bytes`);
          
        } catch (error) {
          console.error(`Error processing line for ${roleName}:`, error);
          // Continue with next line instead of failing the whole request
          // This way we get partial audio rather than nothing
          continue;
        }
      }
    }
    ...
    console.log(`Merging ${audioClips.length} audio clips`);
